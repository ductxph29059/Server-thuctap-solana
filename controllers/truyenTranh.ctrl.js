...
const layDanhSach = async (req, res, next)=>{
    ...
    if(typeof(req.query.tenNV) != 'undefined' && req.query.tenNV != ""){
        match.tenNV = {$regex: req.query.tenNV + "?"}
    }
    if(typeof(req.query.level) != 'undefined' && req.query.level != ""){
        match.level = {$regex: req.query.level + "?"}
    }
...
const themTruyen = async (req, res, next)=>{
    ...
    console.log("cxcx = "+ JSON.stringify(req.body));
    // Validate
    if(typeof tenNV === "string" && tenNV.trim().length === 0){
    ...
    .then(
        (response) => {
            ...
            sock.io.emit("news", data.tenNV + " đã được thêm");
        }
    ).catch((err)=>{
        console.log(err);
